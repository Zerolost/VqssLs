...
function runSign() {
  ...
  checkSignStatus(function(err, status) {
    if (err) {
      log("失败: " + err);
      doNotify("Vomic 签到 v" + VERSION, "失败", err);
      log("========================================");
      return;
    }
    if (status.isSigned) {
    ...
    doSignIn(function(err, r) {
      if (err) {
        log("签到失败: " + err);
        doNotify("Vomic 签到 v" + VERSION, "签到失败", err);
      } else {
        doNotify("Vomic 签到 v" + VERSION, "签到成功",
...
function runCookie() {
  ...
  try {
    ...
    if (old === token) {
      ...
      doNotify("Vomic Cookie v" + VERSION, "Token已更新", "前缀:" + token.substring(0, 20) + "... 长度:" + token.length);
    }
  } catch(e) {
